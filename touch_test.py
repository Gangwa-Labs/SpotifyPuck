...
class Hyperpixel2r:
    ...
    def touch(self, x, y, state):
        if state:
            ...
            if 120 < x < 200:  # Skip backwards button
                sp.previous_track()
            elif 200 < x < 280:  # Pause/Play button
                playback = sp.current_playback()
                if playback and playback['is_playing']:
                    sp.pause_playback()
                else:
                    sp.start_playback()
            elif 280 < x < 360:  # Skip forwards button
                sp.next_track()
    def display_controls_and_info(self):
        ...
        if playback and playback['item']:
            ...
            # Position the text centered and above the buttons
            song_rect = song_surface.get_rect(center=(240, 180))
            artist_rect = artist_surface.get_rect(center=(240, 220))
            ...
            button_positions = [(160, 240), (240, 240), (320, 240)]

            # Draw the buttons on the screen
            for color, position in zip(button_colors, button_positions):
...
display.display_controls_and_info()
running = True
while running:
